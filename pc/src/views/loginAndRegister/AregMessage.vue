    ...
    <div class="regMessage-box">
      ...
      <div class="formBox">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
          <el-form-item :label="$t('lang.regMessage.FirstName')" prop="Firstname">
            <el-input type="text" v-model="ruleForm.Firstname" clearable />
          </el-form-item>
          <el-form-item :label="$t('lang.regMessage.LastName')" prop="Lastname">
      ...
      rules: {
        Firstname:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidateFname, trigger: 'blur' }]
            : [{ validator: validateFname, trigger: 'blur' }],
        Lastname:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidateLname, trigger: 'blur' }]
            : [{ validator: validateLname, trigger: 'blur' }],
        Detailaddress:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidateValue, trigger: 'blur' }]
            : [{ validator: validateValue, trigger: 'blur' }],
        Address:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidateDiZhi, trigger: 'blur' }]
            : [{ validator: validateDiZhi, trigger: 'blur' }],
        Number:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidatePhone, trigger: 'blur' }]
            : [{ validator: validatePhone, trigger: 'blur' }],
        Postcode:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidatetoNum, trigger: 'blur' }]
            : [{ validator: validatetoNum, trigger: 'blur' }],
        address:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidateLength, trigger: 'blur' }]
            : [{ validator: validateLength, trigger: 'blur' }],
        country:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidateSite, trigger: 'blur' }]
            : [{ validator: validateSite, trigger: 'blur' }],
        Email:
          this.$i18n.locale === 'en-us'
            ? [{ validator: EvalidateEMail, trigger: 'blur' }]
        ...
        if (valid) {
          ...
          Fillmsg({
            ...
            if (resp.data.code === 1) {
              ...
              ARegisterSuccess.site = resp.data.data.site
              ARegisterSuccess.email = this.$cookies.get('registerEmail')
              ARegisterSuccess.storeName = this.$cookies.get(
                'registerStoreName'
              )
