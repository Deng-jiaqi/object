  ...
  <div>
    ...
    <main id="printText">
      <el-row>
        <el-col :span="8">
          <!-- <p> -->
          <span class="addset-span">{{$t('printOrder.text1')}}</span>&nbsp;
          <span class="addset-span1">{{orderinfo.ordernum}}</span>
          <br />
          <br />
          <!-- </p> -->
          <!-- <p> -->
          <span class="addset-span">{{$t('printOrder.text2')}}</span>&nbsp;
          <span class="addset-span1">{{orderinfo.name}}</span>
          <br />
          <br />
          <!-- </p>
          <p>-->
          <span class="addset-span">{{$t('printOrder.text3')}}</span>&nbsp;
          <span class="addset-span1">{{orderinfo.address}}</span>
          <!-- </p> -->
        </el-col>
        <el-col :span="8">
          <!-- <p> -->
          <span class="addset-span">{{$t('printOrder.text4')}}</span>&nbsp;
          <span class="addset-span1">{{orderinfo.createtime}}</span>
          <br />
          <br />
          <!-- </p>
          <p>-->
          <span class="addset-span">{{$t('printOrder.text5')}}</span>&nbsp;
          <span class="addset-span1">{{orderinfo.phone}}</span>
          <!-- </p> -->
        </el-col>
      </el-row>
      <div class="table-box">
        <table border="1" cellspacing="0" cellpadding="6" align="left">
          ...
          <tr style="height: 28px" v-for="(item, index) in goods" :key="index">
            <td colspan="5">{{index + 1}}</td>
            <td width="500">{{item.goodsname}}</td>
        ...
        <table cellspacing="0" cellpadding="6" align="left" style="margin: 30px auto 30px auto">
          <tr style="height: 36px; margin-top: 30px">
            <td width="500">
              <span>{{$t('printOrder.td1')}}</span>
              <span class="priceCss">{{getTolPrice.toFixed(2)}}</span>
              {{$t('printOrder.td4')}} +
              <span>{{$t('printOrder.td2')}}</span>
              <span class="priceCss">{{money.shipping}}</span>
              + {{$t('printOrder.td3')}}
              <span>{{money.taxes}}</span>
              - {{$t('printOrder.td5')}}
              <span
                class="priceCss"
              >{{money.discount_amount}}</span>
              {{$t('printOrder.td4')}}
            </td>
          </tr>
        </table>
