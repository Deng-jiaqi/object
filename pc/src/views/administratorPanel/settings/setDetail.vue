<template>
  <div v-loading="loading">
    <p class="add-black" @click="backTop">
      ...
      <span>{{$t('setDetail.header.back')}}</span>
    </p>
    <div class="flexBetween">
      ...
      <el-button
        type="primary"
        @click="addSite"
        v-if="unuse.length + SiteData.length < 4"
      >{{$t('setDetail.header.btn1')}}</el-button>
    </div>
    <el-row>
      <el-col :span="8">
        ...
        <p>
          {{$t('setDetail.content.text2')}}{{unuse.length + SiteData.length}}{{$t('setDetail.content.slot1')}}
      ...
      <el-col :span="15" class="set-two-right">
        ...
        <div>
          <el-link
            ...
            :underline="false"
            @click="clickOrder"
          >{{$t('setDetail.content.text9')}}</el-link>
        </div>
      </el-col>
    </el-row>
    <!--    排序-->
      ...
      <div class="diaBox">
        ...
        <div>
          <ul class="diaSet-order">
            <draggable v-model="getData" @update="datadragEnd1" :options="{animation:500}">
              <li v-for="(item, index) in getData" :key="index" class="cursorLi">
                <span>{{index +1}}.</span>
                {{item.name}}
              </li>
            </draggable>
...
.cursorLi {
  cursor: url("../../../assets/product/tongs.png"), auto;
}
</style>
