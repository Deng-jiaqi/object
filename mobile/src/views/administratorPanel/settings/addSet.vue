<template>
  <div class="addSet" v-loading="loading">
    <div class="addSet-header">
      <h1 class="text-hideThree">{{headerTitle}}</h1>
        ...
        <div class="set-two-right">
          <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm">
                ...
                <el-col :span="24">
                  <el-form-item :label="$t('addSet.label.text4')" prop="country">
                    <el-select v-model="ruleForm.country" style="width: 100%">
                      <el-option v-for="option in arr" :value="option.name" :key="option.value"  :disabled="option.disabled">
                        {{option.name}}
                      </el-option>
                    </el-select>
                  </el-form-item>
                ...
                <el-col :span="24">
                  <el-form-item :label="$t('addSet.label.text5')" prop="Province">
                    <el-select v-model="ruleForm.province" style="width: 100%">
                      <el-option v-for="option in cityArr" :value="option.name" :key="option.value"  :disabled="option.disabled">
                        {{option.name}}
                      </el-option>
                    </el-select>
                  </el-form-item>
            ...
            <el-form-item :label="$t('addSet.content.label1')" prop="phone">
              <el-input :placeholder="$t('addSet.content.place2')" v-model="ruleForm.phone" class="input-with-select">
                <div class="addSet-select" slot="append">
                  <el-select v-model="ruleForm.code" :placeholder="$t('addSet.content.place3')">
                    <template v-if="this.$i18n.locale === 'zh-cn'">
                      <el-option v-for="(item,index) in ruleForm.areaCode"
                                 :key="index"
    ...
    <div v-if="field !== 'addSet'">
      <el-row class="set-twoBox"  v-if="len > 1 || is_use === 0">
        <el-col :span="24">
          ...
          <p>{{is_use === 1 ? blockData.text2: blockData1.text2}}
            <el-link type="primary" :underline="false">{{is_use === 1 ? blockData.text3:blockData1.text3}}</el-link>
          </p>
        </el-col>
        <el-col :span="24">
          <div class="set-two-right">
    ...
    <div class="flexBetween set-twoBox">
      <div>
        <template v-if="field !== 'addSet'">
          <el-button type="danger" @click="delSite" v-if="is_use === 0">{{$t('addSet.content.btn1')}}</el-button>
        </template>
      </div>
    ...
    <el-dialog
      ...
      <div class="dead-replace" v-else>
        <div>
          <p class="el-icon-info">
            ...
            <span style="color: black;font-weight: bold">{{headerTitle}}</span>
            {{$t('addSet.elDialogSite.text3')}}</p>
        </div>
        <div>
      ...
      <el-button type="primary" @click="blockSave" v-loading="loading">{{$t('addSet.elDialogSite.btn2')}}</el-button>
    </span>
    </el-dialog>
    <!--    删除位置-->
    <el-dialog
        ...
        width="100%">
      <div class="diaBox">
        <p>{{$t('addSet.elDialogDel.text1')}} <span>{{ruleForm.name}}</span>{{$t('addSet.elDialogDel.text2')}}</p>
      </div>
      <span slot="footer" class="dialog-footer">
...
import {
  validatetoNum, validatePhone
} from '../../../common/validator'
import {
  EvalidatePhone, EvalidatetoNum
