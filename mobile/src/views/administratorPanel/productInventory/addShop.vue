  ...
  <div class="add-box" v-loading="loading">
    <div class="add-shop">
      <div class="add-header">
        <p class="left">{{$t('addShop.header.title')}}</p>
        <p class="right" @click="backTop">
          <span class="iconfont icon-fanhui1"></span>
          {{$t('addShop.header.back')}}
        </p>
      </div>
      <el-form class="add-form" ref="ruleForm" :rules="rules" :model="ruleForm">
                ...
                >
                  ...
                  <div class="mask">
                    <el-tooltip
                      class="item"
                      effect="dark"
                      :content="$t('addShop.proImg.content1')"
                      placement="top"
                    >
                    ...
                    <el-tooltip
                      class="item"
                      effect="dark"
                      :content="$t('addShop.proImg.content2')"
                      placement="top"
                    >
                      <span @click="del()" class="Delete">
              ...
              <el-row>
                <el-col :span="24">
                  <el-form-item prop="price">
                    ...
                    <p v-if="ruleForm.price > 0 && ruleForm.costprice !== ''">
                      <span>{{$t('addShop.price.text4')}}</span>
                      {{profit.priceTwo}}%
                    </p>
                  </el-form-item>
                </el-col>
        ...
        <el-form-item>
          ...
          <div class="hide-tail">
            <el-checkbox v-model="gzCheckbox">{{$t('addShop.repertory.text3')}}</el-checkbox>
            <br />
            <el-checkbox v-if="gzCheckbox === true">{{$t('addShop.repertory.text4')}}</el-checkbox>
          </div>
              ...
              <table width="100%">
                ...
                <tbody ref="Tbody">
                  <tr class="attributes-tr" v-for="(item,index) in addData" :key="item.value">
                    ...
                    <td @click="DeleteTr(index)" v-if="addData.length!==1">
                  ...
                  <tr class="attributes-tr" v-if="this.addData.length!==3">
                    <el-button
                      ...
                      @click="addShuXin"
                    >{{$t('addShop.multiAttribute.btn')}}</el-button>
                  </tr>
                </tbody>
              </table>
                      ...
                      <div>
                        <label class="flexCss">
                          ...
                          <span v-if="getTotal.length<1">
                            &nbsp;&nbsp;
                            <span>{{$t('addShop.multiAttribute.showText1')}}</span>
                            {{editData.length}}
                            <span>{{$t('addShop.multiAttribute.showText2')}}</span>
                          </span>
                          <span v-else>
                            &nbsp;&nbsp;
                            <span>{{$t('addShop.multiAttribute.showText3')}} {{getTotal.length}} {{$t('addShop.multiAttribute.showText4')}}</span>
                            <el-popover placement="bottom" trigger="click">
                              ...
                              <el-button
                                slot="reference"
                                class="el-icon-edit"
                              >{{$t('addShop.multiAttribute.btn2')}}</el-button>
                            </el-popover>
                          </span>
                        </label>
                      </div>
                      <div class="edit-popover" v-if="getTotal.length<1"></div>
                    </div>
                          ...
                          <div>
                            ...
                            <template v-for="(data, i) in item.location">
                              <span
                                ...
                                :key="i"
                              >{{data.num}} {{$t('addShop.multiAttribute.text5')}}</span>
                            </template>
                          </div>
                        </div>
                      </li>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <button
          ...
          ref="BtnCss"
          class="confirmBtn"
        >{{$t('addShop.elDialogSite.btn2')}}</button>
      </span>
    </el-dialog>
    <!--    编辑价格-->
    <el-dialog
    ...
    >
      ...
      >
        <div>
          <div>
            <div class="editNum">
              <p>
                {{$t('addShop.elDialogLocation.span1')}}
                <span>{{attrsName}}</span>
                {{$t('addShop.elDialogLocation.span2')}}
              </p>
      ...
      <span slot="footer" class="dialog-footer">
        ...
        >{{$t('addShop.elDialogLocation.btn4')}}</el-button>
      </span>
    </el-dialog>
    <!--    编辑SKU-->
    <el-dialog
        ...
        <div>
          <div class="editOptions-edits-box1" v-for="(item, index) in attrsData" :key="index">
              ...
              <el-col :span="24">
                <div class="editOptions-edits-box1-d">
                  ...
                  <span
                    ...
                    v-show="hidecost"
                  >{{$t('addShop.elDialogEdit.span2')}}{{parseFloat((item.price - item.costprice) / item.costprice * 100).toFixed(1)}}%</span>
                </div>
              </el-col>
              <el-col :span="24">
                <div class="editOptions-edits-box1-d">
            ...
            <div class="editOptions-edits-box2">
              <p>
                <span>{{$t('addShop.elDialogEdit.span6')}}</span>
                <span>{{$t('addShop.elDialogEdit.span7')}}</span>
              </p>
              <p>
                <span>{{$t('addShop.elDialogEdit.span8')}}</span>
                <span>{{$t('addShop.elDialogEdit.span9')}}</span>
              </p>
              <ul>
            ...
            <div class="btnFooter">
              ...
              <el-button
                type="primary"
                @click="saveRepertory(item)"
                v-preventReClick="2000"
              >{{$t('addShop.elDialogEdit.btn2')}}</el-button>
            </div>
          </div>
        </div>
      </div>
...
import {
  ...
  toolbarOptions,
  titleConfigEn
} from '../../../common/quillEditorS' // 引入富文本提示和部分功能
...
export default {
  ...
  data () {
    var validateSku = (rule, value, callback) => {
      if (value !== '') {
        if (this.ruleForm.sku === this.ruleForm.barCode) {
    ...
    var validateCode = (rule, value, callback) => {
      if (value !== '') {
        if (this.ruleForm.barCode === this.ruleForm.sku) {
    ...
    return {
      ...
      rules: {
        ...
        barCode: [{ validator: validateCode, trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    fileChange (e) {
      ...
      if (leng + _this.ruleForm.imgs.length > _this.ruleForm.maxCount) {
        alert(
          this.$t('addShop.hint.text1') +
          _this.ruleForm.maxCount +
          this.$t('addShop.hint.text2')
        )
        return
      }
    ...
    ArrMethods () {
      ...
      // ===========================================
      for (let i = 0; i < this.shuzunum.length; i += this.resultSize) {
        // 拆分数组
    ...
    save (formName) {
      ...
      for (let v of this.addData) {
        // 判断有多属性的情况，没有输入多属性，给提示，不能添加或者不添加多属性
  ...
  computed: {
    ...
    profit: function () {
      ...
      priceTwo =
        ((this.ruleForm.price - this.ruleForm.costprice) /
          this.ruleForm.price) *
        100
      return {
    ...
    totalSumAll: function () {
      ...
      this.editData.forEach(val => {
        val.location.forEach(data => {
          totalSumAll += data.num
          data.sum = totalSumAll
        })
      })
      return totalSumAll
    }
