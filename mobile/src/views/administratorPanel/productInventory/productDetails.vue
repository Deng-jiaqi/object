<template>
  <div class="productDetails" v-loading="loading">
    <div class="productDetails-header">
      <div class="header-h1">{{goodsGetName}}</div>
      <div class="add-black" @click="backTop">
        <span class="el-icon-arrow-left"></span>
        {{$t('productDetails.header.back')}}
      </div>
    </div>
    <div class="productDetails-form">
      <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" class="add-form">
            ...
            <div class="addUp-box">
              ...
              <div class="img-files flex">
                <div class="img-file" v-if="imgs.length < maxCount">
                  ...
                  >{{$t('productDetails.proImg.shopUp')}}</label>
                </div>
              </div>
            </div>
            <div class="up-img-box">
              <div class="up-content" v-if="imgs.length>0">
                <div
                  class="up-datu"
                  v-if="imgs.length>0"
                  v-dragging="{ item: imgs[0], list: imgs, group: 'item'}"
                >
                  <div class="up-ImgBox">
                  ...
                  <div class="mask">
                    <el-tooltip
                      class="item"
                      effect="dark"
                      :content="$t('productDetails.proImg.content1')"
                      placement="top"
                    >
                    ...
                    <el-tooltip
                      class="item"
                      effect="dark"
                      :content="$t('productDetails.proImg.content2')"
                      placement="top"
                    >
                      <span @click="del(imgs[0],imgs[0])" class="Delete">
                  ...
                  >
                    ...
                    <div class="mask-xiao">
                      ...
                      <el-tooltip
                        ...
                        placement="top"
                      >
                        <span @click="del(i, item)" class="Delete">
                          <i class="el-icon-delete"></i>
                        </span>
                      </el-tooltip>
                    </div>
        ...
        <!-- 上传视频 -->
        <div class="c-video-upload">
          <p class="name-title">{{$t('addShop.text1')}}</p>
          <div class="video-upload-box">
            <el-upload
              action="/Shop/public/index.php/Product/Goods/UploadVideo"
              :show-file-list="false"
            ...
            >
              <span
                ...
                v-show="videoUploadStep==='notStart'||videoUploadStep==='fail'"
              >{{$t('addShop.text2')}}</span>
            </el-upload>
            <el-image
              v-show="videoUploadStep==='notStart'||videoUploadStep==='fail'"
              :title="$t('addShop.text3')"
              class="video-img"
              :src="videoImg"
              lazy
            ></el-image>
            <el-progress
              v-show="this.videoUploadStep==='process'"
              class="upload-video-progress"
              type="circle"
              color="#4267b2"
              :percentage="videoUploadpercentage"
            ></el-progress>
            <video controls="controls" :src="videoSrc" v-show="videoUploadStep==='success'"></video>
            <span
              class="iconfont iconlajitong1"
              v-show="videoUploadStep==='success'"
              @click="deleteVideo"
            ></span>
          </div>
        </div>
              ...
              <el-row>
                <el-col :span="11">
                  <el-form-item prop="price">
                    ...
                    >{{$t('productDetails.price.text4')}}{{profit.priceTwo}}%</p>
                  </el-form-item>
                </el-col>
        ...
        <el-form-item v-if="is_more_attribute === 0">
          <div class="multiple-box">
            ...
            <div class="checkBox-css">
              <el-checkbox v-model="gzCheckbox">{{$t('productDetails.repertory.text3')}}</el-checkbox>
              <br />
              <el-checkbox v-if="gzCheckbox === true">{{$t('productDetails.repertory.text4')}}</el-checkbox>
            </div>
            <div class="mulBox4">
              ...
              <div>
                ...
                <span
                  class="editWarehouse-text"
                  @click="bjSite"
                >{{$t('productDetails.repertory.text6')}}</span>
              </div>
            </div>
            <div class="mulBox4">
                ...
                <div>
                  ...
                  <el-popover placement="bottom" v-model="item.visibleEditSet" trigger="click">
                    ...
                    <div class="editeCaoZuo">
                      ...
                      >{{$t('productDetails.repertory.btn1')}}</el-button>
                      <el-button
                        @click="saveNewNum(item)"
                        type="primary"
                      >{{$t('productDetails.repertory.btn2')}}</el-button>
                    </div>
        ...
        <el-form-item v-if="is_more_attribute === 1">
          <div class="productDetails-moreSx">
                ...
                <el-popover placement="bottom" trigger="click">
                  <p slot="reference" class="qiTaCaoZuo">
                    <el-link :underline="false">
                      {{$t('productDetails.haveMultiAttribute.span2')}}
                      <span
            ...
            <div>
              <div>
                <label class="flexCssLabel">
                  ...
                  <span v-else class="input-checkbox-el">
                    &nbsp;&nbsp;{{$t('productDetails.haveMultiAttribute.span5')}} {{getTotal.length}}
                    ...
                    <el-button-group>
                      ...
                      <el-popover visible-arrow="false" placement="bottom" trigger="click">
                        <el-button slot="reference">
                          {{$t('productDetails.haveMultiAttribute.btn2')}}
                          <span
                            class="el-icon-caret-bottom"
                          ></span>
                        </el-button>
              ...
              <div class="BianJiBox" v-if="getTotal.length < 1">
                <el-popover placement="bottom" v-model="visibleWeiZhi" trigger="click">
                  <el-button slot="reference">
                    {{weiZhi}}
            ...
            <ul>
              <li
                v-for="(item, index) in multiattributeData"
                :key="index"
              ...
              >
                <div>
                  ...
                  <div>
                    <p class="text-hideTwo">{{item.attr_value}}</p>
                    <p>{{item.sku}}</p>
                  </div>
                </div>
            ...
            <div class="allNum">
              <p>
                <span>{{weiZhi}}</span>
                {{$t('productDetails.haveMultiAttribute.span7')}}
              </p>
              <p>
                <span>{{totalstorage}}</span>
                {{$t('productDetails.haveMultiAttribute.span8')}}
              </p>
            </div>
          </div>
        </el-form-item>
              ...
              <table width="100%">
                ...
                <tbody class="attributes-xuanXiang" ref="Tbody">
                  <tr class="attributes-tr" v-for="(item,index) in MuladdData" :key="item.value">
                    <td>
                      <input type="text" class="carriage-input" v-model="item.attribute" />
                    </td>
                    <td>
                      <div :class="item.judge === true ?  inputBorder[0] : inputBorder[1]">
                        <ul class="attributes-ul" style="margin: 1px">
                          <li
                            v-for="(i,idx) in item.optionSpan"
                            :key="idx"
                            :class="index===0 ? sstt[0] : index === 1 ? sstt[1] :sstt[2] "
                          >
                            <span class="attributes-option">{{i}}</span>
                            <span class="attributes-close" @click="DeleteSpan(index,idx,i)">
                              <span class="iconfont iconclose" style="cursor: pointer"></span>
                            </span>
                          </li>
                          <li>
                            <input
                    ...
                    <td @click="DeleteTr(index)" v-if="MuladdData.length!==1">
                      <span class="iconfont iconlajitong" style="font-size: 20px;cursor: pointer"></span>
                    </td>
                  </tr>
                  <tr class="attributes-tr" v-if="MuladdData.length!==3">
                    <el-button
                      ...
                      @click="MuladdShuXin"
                    >{{$t('productDetails.noMultiAttribute.btn')}}</el-button>
                  </tr>
                </tbody>
              </table>
                    ...
                    <div class="edit-site-box">
                      <div>
                        <label class="flexCssLabel">
                          ...
                          />
                          <div v-if="MulgetTotal.length<1">
                            &nbsp;&nbsp;{{$t('productDetails.noMultiAttribute.showText1')}}
                            {{editData.length}} {{$t('productDetails.noMultiAttribute.showText2')}}
                          </div>
                          <div v-else>
                            &nbsp;&nbsp;
                            <span>
                              {{$t('productDetails.noMultiAttribute.showText3')}}
                              {{MulgetTotal.length}} {{$t('productDetails.noMultiAttribute.showText4')}}
                            </span>
                            <el-popover
                              class="input-checkbox-el"
                              placement="bottom"
                              trigger="click"
                            >
                              <div class="compile">
                                <ul class="locationlistCss">
                                  <li
                                    @click="MulbjPrice"
                                  >{{$t('productDetails.noMultiAttribute.li1')}}</li>
                                  <li
                                    @click="MuldialogVisibleLocation = true"
                                  >{{$t('productDetails.noMultiAttribute.li2')}}</li>
                                  <li
                                    @click="MulbjSku"
                                  >{{$t('productDetails.noMultiAttribute.li3')}}</li>
                                  <li
                                    @click="MulbjCode"
                                  >{{$t('productDetails.noMultiAttribute.li4')}}</li>
                                </ul>
                              </div>
                              <el-button
                                slot="reference"
                                class="el-icon-edit"
                              >{{$t('productDetails.noMultiAttribute.btn2')}}</el-button>
                            </el-popover>
                          </div>
                        </label>
                      </div>
                      <div class="edit-popover" v-if="MulgetTotal.length<1"></div>
                    </div>
                      ...
                      >
                        <div>
                          <input
                            class="check_box tui-checkbox"
                            type="checkbox"
                            v-bind:checked="item.MulisSelect"
                        ...
                        <div>
                          ...
                          <div>
                            ...
                            <template v-for="(data, i) in item.location">
                              <span
                                v-if="qhCangkuName === data.name"
                                :key="i"
                              >{{data.num}} {{$t('productDetails.noMultiAttribute.text5')}}</span>
                            </template>
                          </div>
                        </div>
                      </li>
      ...
      <div class="btnCss">
        ...
        >{{$t('productDetails.delBtn')}}</el-button>
        <el-button
          @click="saveMultiple('ruleForm')"
          v-preventReClick="2000"
        >{{$t('productDetails.saveBtn')}}</el-button>
      </div>
    </div>
    <!--    ===================================有多属性的模态框-->
    <div>
      ...
      >
        <div class="editWarehouse-box2" v-show="judgeSite">
          <p>
            ...
            {{$t('productDetails.haveElDialogSite.text1')}}
          </p>
        </div>
        ...
        <ul>
          <li v-for="(item, index) in checkedEdit" :key="index" class="checkBox-css">
            ...
            <p v-if="item.checked === false">
              <span
                v-if="item.num !==0"
              >{{$t('productDetails.haveElDialogSite.text3')}} {{item.num}} {{$t('productDetails.haveElDialogSite.text4')}}</span>
            </p>
          </li>
        </ul>
        <span slot="footer" class="dialog-footer">
          ...
          >{{$t('productDetails.haveElDialogSite.btn1')}}</el-button>
          <button
            type="button"
            @click="addWarehouse"
            ref="BtnCss"
            v-preventReClick="2000"
            class="confirmBtn"
          >{{$t('productDetails.haveElDialogSite.btn2')}}</button>
        </span>
      </el-dialog>
      <!--    编辑价格-->
      <el-dialog
      ...
      >
              ...
              <li v-for="(item, index) in Editprice" :key="index">
                ...
                <div>
                  ...
                  <p v-if="item.price > 0">
                    ...
                    {{$t('productDetails.haveElDialogPrice.text2')}}
                    {{$t('productDetails.haveElDialogPrice.text3')}}{{parseFloat((item.price-item.costprice)/item.price*100).toFixed(2)}}
                  </p>
                </div>
              </li>
            </ul>
        ...
        <span slot="footer" class="dialog-footer">
          ...
          >{{$t('productDetails.haveElDialogPrice.btn3')}}</el-button>
        </span>
      </el-dialog>
      <!--    编辑选项值-->
      <el-dialog
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button
            ...
            @click="saveAttr"
            style="background-color: #4267B2"
          >{{$t('productDetails.haveElDialogOptions.btn2')}}</el-button>
        </span>
      </el-dialog>
      <!--    选择编辑数量-->
      <el-dialog
      ...
      >
        ...
        >
          <div>
            <div v-if="attrs.length > 0">
              <div class="editNum">
                <p>
                  {{$t('productDetails.haveElDialogLocation.span1')}}
                  <span
                    style="color: black;font-weight: bold"
                  >{{attrsName}}</span>
                  {{$t('productDetails.haveElDialogLocation.span2')}}
                </p>
              ...
              <ul class="editPrice-ul">
                <li v-for="(item, index) in attrs" :key="index">
                  ...
                  <div class="priceInput">
                    ...
                    <span
                      v-else
                    >{{$t('productDetails.haveElDialogLocation.span3')}} {{attrsName}}{{$t('productDetails.haveElDialogLocation.span4')}}</span>
                  </div>
                </li>
              </ul>
            </div>
            <p
              v-else
            >{{$t('productDetails.haveElDialogLocation.span5')}} {{attrsName}} {{$t('productDetails.haveElDialogLocation.span6')}}</p>
          </div>
          <span slot="footer" class="dialog-footer">
            <el-button
              @click="dialogVisibleNum = false"
            >{{$t('productDetails.haveElDialogLocation.btn2')}}</el-button>
            <el-button
              type="primary"
              @click="saveNum"
              style="background-color: #4267B2"
            >{{$t('productDetails.haveElDialogLocation.btn3')}}</el-button>
          </span>
        </el-dialog>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button
            @click="dialogVisibleLocation = false"
          >{{$t('productDetails.haveElDialogLocation.btn2')}}</el-button>
          <el-button
            type="primary"
            @click="dialogVisibleLocation = false"
          >{{$t('productDetails.haveElDialogLocation.btn4')}}</el-button>
        </span>
      </el-dialog>
      <!--    编辑选项-->
      <el-dialog
      ...
      >
        <div class="bjOptionsHint">
          <p>
            <img src alt />
            {{$t('productDetails.haveElDialogOption.text1')}}
          </p>
        </div>
          ...
          <li v-for="(item, index) in addData" :key="index">
            <div>
              ...
              <div>
                <el-input
                  ...
                  v-if="item.judge === false"
                  class="bianjiOption-box"
                  :placeholder="item.placeholder"
                ></el-input>
                <ul class="option-ul" v-else>
                  <li v-for="(it, idx) in item.attrs" :key="idx">
                    <div>
                      <span>{{it.attr_value}}</span>
                    </div>
                    <div v-if="item.attrs.length>1" @click="delattrsX(index,idx)">
                      <span>x</span>
                    </div>
                  </li>
                </ul>
            ...
            <div>
              <span v-if="item.attrs.length<=1">
                ...
                >{{$t('productDetails.haveElDialogOption.text2')}}</span>
              </span>
            </div>
          </li>
        </ul>
        <el-button
          class="addQiTa"
          @click="addShuXin"
          v-if="addData.length!==3"
        >{{$t('productDetails.haveElDialogOption.text3')}}</el-button>
        <span slot="footer" class="dialog-footer">
          <el-button @click="OptionNo">{{$t('productDetails.haveElDialogOption.btn1')}}</el-button>
          <el-button
            type="primary"
            @click="saveOption"
          >{{$t('productDetails.haveElDialogOption.btn2')}}</el-button>
        </span>
      </el-dialog>
      <!--    编辑上传多属性图片-->
      <el-dialog
        ...
        <span slot="footer" class="dialog-footer">
          <div class="leftBtn-footer">
            <div>
              <el-button
                type="danger"
                v-if="attrImg !== require('../../../assets/addShop/imgdiushi.png')"
                @click="delImg"
              >{{$t('productDetails.haveElDialogImg.btn1')}}</el-button>
            </div>
            <div>
              <el-button class="editAddImg" @click="uploadHint">
                {{$t('productDetails.haveElDialogImg.btn4')}}
                <template
                  v-if="imgs.length < maxCount"
                >
                  <input
                    type="file"
                    id="filesEdit"
              ...
              <el-button
                type="primary"
                @click="upImgs"
              >{{$t('productDetails.haveElDialogImg.btn3')}}</el-button>
            </div>
          </div>
        </span>
      </el-dialog>
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button
            type="primary"
            @click="saveEditSku"
          >{{$t('productDetails.haveElDialogSku.btn2')}}</el-button>
        </span>
      </el-dialog>
      <!--    编辑条码-->
      <el-dialog
      ...
      >
        ...
        <span slot="footer" class="dialog-footer">
          ...
          <el-button
            type="primary"
            @click="saveEditCode"
          >{{$t('productDetails.haveElDialogCode.btn2')}}</el-button>
        </span>
      </el-dialog>
    </div>
    <!--    ============================================================-->
    ...
    <div>
      ...
      >
        <div>
          <div>
            <div class="editPrice">
              ...
              <div class="priceInput">
                <span class="priceSpanTwo">￥</span>
                <input type="text" placeholder="100.00" v-model="editPrice" @blur="myPrice" />
            ...
            <ul class="editPrice-ul">
              <li v-for="(item,index) in ulData" :key="index">
                ...
                <div>
                  ...
                  <p v-if="item.price > 0">
                    <span>{{item.costprice}}</span>
                    {{$t('productDetails.elDialogPrice.text2')}}
                    {{$t('productDetails.elDialogPrice.text3')}}{{parseFloat((item.price-item.costprice)/item.price*100).toFixed(2)}}
                  </p>
                </div>
              </li>
            </ul>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button
            @click="MuldialogVisiblePrice = false"
          >{{$t('productDetails.elDialogPrice.btn2')}}</el-button>
      ...
      >
        ...
        >
          <div>
            <div>
              <div class="editNum">
                <p>
                  {{$t('productDetails.elDialogLocation.span1')}}
                  <span>{{MulattrsName}}</span>
                  {{$t('productDetails.elDialogLocation.span2')}}
                </p>
          ...
          <span slot="footer" class="dialog-footer">
            <el-button
              @click="MuldialogVisibleStore = false"
            >{{$t('productDetails.elDialogLocation.btn2')}}</el-button>
            <el-button
              type="primary"
              @click="MulsaveNum"
            >{{$t('productDetails.elDialogLocation.btn3')}}</el-button>
          </span>
        </el-dialog>
        <div class="editOptions-num">
          ...
          <ul>
            <li
              v-for="(item, index) in MulcangkuData"
              :key="index"
              @click="MulbianJiNum(item,index)"
            >
              <span>{{item.name}}</span>
              <span class="el-icon-arrow-right"></span>
            </li>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button
            @click="MuldialogVisibleLocation = false"
          >{{$t('productDetails.elDialogLocation.btn2')}}</el-button>
          <el-button
            type="primary"
            @click="MuldialogVisibleLocation = false"
          >{{$t('productDetails.elDialogLocation.btn4')}}</el-button>
        </span>
      </el-dialog>
      <!--    编辑SKU-->
      <el-dialog
        ...
        <div class="bjSku">
          <ul>
            <li v-for="(item, index) in MulskuData" :key="index">
              <p class="text-hideTwo">{{item.name.join('-')}}</p>
              <el-input v-model="item.sku"></el-input>
            </li>
          </ul>
        </div>
      ...
      >
        ...
        <div slot="footer" class="dialog-footer">
          <el-button
            @click="MuldialogVisibleCode = false"
          >{{$t('productDetails.elDialogCode.btn1')}}</el-button>
        ...
        <div class="editOptions-edits">
          <div>
            <div class="editOptions-edits-box1" v-for="(item, index) in attrsData" :key="index">
                ...
                <el-col :span="24">
                  <div class="editOptions-edits-box1-d">
                    ...
                    <span
                      ...
                      v-show="hidecost"
                    >
                      {{$t('productDetails.elDialogEdit.span2')}}
                      {{parseFloat((item.price - item.costprice) / item.costprice * 100).toFixed(1)}}%
                    </span>
                  </div>
                </el-col>
                <el-col :span="24">
              ...
              <div class="priceBox">
                <el-row>
                  <el-col :span="24" class="editOptions-edits-box1-d">
                    <div>
              ...
              <div class="editOptions-edits-box2">
                <p>
                  <span>{{$t('productDetails.elDialogEdit.span6')}}</span>
                  <span>{{$t('productDetails.elDialogEdit.span7')}}</span>
                </p>
                <p>
                  <span>{{$t('productDetails.elDialogEdit.span8')}}</span>
                  <span>{{$t('productDetails.elDialogEdit.span9')}}</span>
                </p>
                <ul>
              ...
              <div class="btnFooter">
                <el-button
                  @click="MuldialogVisibleEdit = false"
                >{{$t('productDetails.elDialogEdit.btn1')}}</el-button>
                <el-button
                  type="primary"
                  @click="saveRepertory(item, index)"
                >{{$t('productDetails.elDialogEdit.btn2')}}</el-button>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  data () {
    var validateSku = (rule, value, callback) => {
      if (value !== '') {
        if (this.ruleForm.sku === this.ruleForm.itemcode) {
          callback(new Error('SKU和code不能相同'))
        } else {
          callback()
        }
      } else {
    ...
    var validateitemcode = (rule, value, callback) => {
      if (value !== '') {
        if (this.ruleForm.itemcode === this.ruleForm.sku) {
          callback(new Error('code不能SKU相同'))
        } else {
          callback()
        }
      } else {
    ...
    return {
      ...
      Btnloading: true, // 加载时
      hideBtn: true, // 加载出来前隐藏
      videoUploadpercentage: 0,
      videoUploadStep: 'notStart', // 视频上传处于哪一部
      videoSrc: '',
      videoImg: require('@/assets/addShop/video.png'),
      inputBorder: [
        'carriageInputTwo',
        'attributes-option-boxTwo'
      ],
      // checkedCities: '',
      loading: true,
      collect: true, // 税收
      ...
      rules: {
        ...
        sku: [{ validator: validateSku, trigger: 'blur' }],
        itemcode: [{ validator: validateitemcode, trigger: 'blur' }]
      },
  ...
  methods: {
    beforevideoUpload (file) { // 文件上传前检查
      const limit50m = file.size / 1024 / 1024 < 50
      console.log(file.name)
      let fileType = last(file.name.split('.'))
      const isVideo = ['dat', 'asf', 'rm', 'ram', '3gp', 'mov', 'm4v', 'dvix', 'dv', 'qt', 'divx', 'cpk', 'fli', 'flc', 'mod', 'mp4', 'wmv', 'flv', 'avi', 'mkv', 'vob', 'mpg', 'rmvb', 'mpeg', 'mov', 'mts', 'DAT', 'ASF', 'RM', 'RAM', '3GP', 'MOV', 'M4V', 'DVIX', 'DV', 'QT', 'DIVX', 'CPK', 'FLI', 'FLC', 'MOD', 'MP4', 'WMV', 'FLV', 'AVI', 'MKV', 'VOB', 'MPG', 'RMVB', 'MPEG', 'MOV', 'MTS'].indexOf(fileType)
      if (!limit50m) {
        this.$message.error(this.$t('addShop.text4'))
        return false
      }
      if (isVideo === -1) {
        this.$message.error(this.$t('addShop.text5'))
        return false
      }
    },
    handleVideoUpload (file) { // 文件状态改变时的钩子，添加文件、上传成功和上传失败时都会被调用(用的是直接上传没有添加文件这步)
      console.log(file)
      if (file.status === 'fail') {
        this.videoUploadStep = 'fail'
        this.$message.error(this.$t('addShop.text6'))
      }
      if (file.status === 'success') {
        this.videoUploadpercentage = 100 // 请求完成把进度调到100
        setTimeout(() => {
          this.videoUploadStep = 'success'
          this.videoSrc = file.response.data.url
        }, 2000)
      }
    },
    uploadVideoProcess (event, file, fileList) { // 文件上传时的钩子
      this.videoUploadStep = 'process'
      console.log(event)
      this.videoUploadpercentage = Math.floor(event.percent)
      if (this.videoUploadpercentage === 100) { // 解析视频完成后把进度调到99
        this.videoUploadpercentage = 99
      }
    },
    deleteVideo () {
      this.$confirm(this.$t('addShop.text7'), this.$t('addShop.text8'), {
        confirmButtonText: this.$t('addShop.text9'),
        cancelButtonText: this.$t('addShop.text10'),
        type: 'warning'
      }).then(() => {
        this.videoUploadStep = 'notStart'
        this.videoUploadpercentage = 0
        this.videoSrc = ''
        this.$refs.videoUpload.clearFiles()
        this.$message({
          type: 'success',
          message: this.$t('addShop.text11')
        })
      })
    },
    // 富文本提示
    ...
    saveEditCode () {
      ...
      saveitemcode({
        ...
        this.multiattributeData = resp.data.data.attr_location.info
        this.dialogVisibleCode = false
      })
    },
    // 有多属性确认保存sku
    ...
    funfa () {
      ...
      this.editData.forEach((val, index) => {
        val.location.forEach((v) => {
          this.editData[index].repertoryNum += v.num
        })
      })
    },
    // 点击编辑价格
    ...
    saveRepertory (item) {
      ...
      item.repertoryNum = num
      item.site = item.location.length
        ...
        this.Mulattrs.forEach((val, idx) => {
          this.editData.forEach((data, index) => {
            if (this.Mulattrs[idx].siteId === this.editData[index].siteId) {
              ...
              data.repertoryNum = num
              data.site = data.location.length
            }
          })
        })
      } else {
      ...
      this.$confirm(this.$t('productDetails.hint.text7'), this.$t('productDetails.hint.btn1'), {
        ...
        deleteattr({
          ...
          if (resp.data.data.length < 1) {
            ...
            this.multiattributeData = resp.data.data.attr_location.info
            this.$message.success(this.$t('productDetails.hint.btn4'))
          }
        })
      }).catch(() => {
        this.$message.info(this.$t('productDetails.hint.btn5'))
      })
    },
    // 点击编辑价格
    bjPrice () {
    ...
    saveNewNum (item) {
      if (this.editSetNumActive === 0) {
        ...
        if (this.SetnewNum === '') {
          ...
          item.num = parseInt(this.SetnewNum)
          this.SetnewNum = ''
        }
      }
      item.visibleEditSet = false
    ...
    saveMultiple (formName) {
      let num = Number
      let judge = false // 判断属性值是否输入完成
      if (this.is_more_attribute === 1) { // 有多属性的保存
        ...
        for (let v of this.MuladdData) { // 判断有多属性的情况，没有输入多属性，给提示，不能添加或者不添加多属性
          if (v.optionSpan.length > 0 || this.attrTable === false) {
            judge = true // 多属性值未输入完成
            v.judge = false // 提示让输入框变红
          } else {
            judge = false
            v.judge = true
            break
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (judge === true) {
              ...
              this.$message.error(this.$t('productDetails.hint.text14'))
            }
          } else {
            this.$message(this.$t('productDetails.hint.text12'))
          }
