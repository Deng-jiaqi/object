  ...
  <el-row>
    <el-col :span="24">
      <div class="allProducts" v-if="getLengths > 0" v-loading="loading">
        <div class="header">
          <h2 class="header-h1">{{$t('product.header.title')}}</h2>
          <el-button
            type="primary"
            style="background-color: #385CA5"
            @click="addChanPin"
          >{{$t('product.header.btn')}}</el-button>
        </div>
        <!--    头部搜索-->
        <div class="header-search">
          ...
          <div class="div-search">
            <input
              class="search"
              v-model="input"
              :placeholder="$t('product.header.search')"
              @input="souSuo"
            />
            ...
            <button
              ...
              @click="editProCP"
              class="serBtn"
            >{{$t('product.edit.text')}}({{getTotal.length}})</button>
            <el-popover
              placement="bottom"
              v-model="visibleBj"
              v-if="getTotal.length>0"
            ...
            >
              <div class="btn1-bianJi">
                <ul>
                  <li
                    v-for="(item,index) in $t('product.edit.text2.list')"
                    :key="index"
        ...
        <div class="inventory-table">
          <table v-if="judgeLength>0" v-loading="loading">
            <thead>
              ...
              <th class="input-th1">{{$t('product.table.thead.th1')}}</th>
              <th>{{$t('product.table.thead.th2')}}</th>
              <!--            <th>{{$t('product.table.thead.th3')}}</th>-->
              <th>{{$t('product.table.thead.th4')}}</th>
            </thead>
            <tbody style="text-align: center">
              <tr v-for="(item, index) in allProducts" :key="index" @click="routerDetails(item)">
                ...
                <td class="td-imgBox">
                  <div>
                    <img :src="item.smimg === null ? missingImages : item.smimg" alt />
                  </div>
                  <div class="td-text-hide">
                    <p class="text-hideTwo">{{item.goodsname}}</p>
                    <p>{{item.status}}</p>
                  </div>
                </td>
                <td>
                  <p class="text-hide">
                    {{item.inventory}}
                    <span>
                      <span>{{item.multinum}}</span>
                      {{$t('product.table.tbody.text')}}
                    </span>
                  </p>
                </td>
                <td>
        ...
        >
          <div class="mtl-box">
            <el-input
              :placeholder="$t('product.elDialog.place')"
              v-model="serchProducts"
      ...
      <div class="allProducts-two" v-else>
        <div class="allProducts-header">
          <h1 class="header-h1">{{$t('product.header.title')}}</h1>
        </div>
        <el-row :gutter="20" class="allProducts-content">
          <el-col :span="24" class="allProducts-content-img">
...
export default {
  ...
  data () {
    return {
      missingImages: require('../../../assets/addShop/tupian.png'),
      timer: '', // 定时器
      getLengths: 1,
    ...
    clickmsg (item, index) {
      ...
      if (index === 0 || index === 1) {
        ...
        this.$confirm(this.$t('product.hint.text1'), this.$t('product.hint.text2'), {
          ...
          this.$message({ type: 'info', message: this.$t('product.hint.text5') })
        })
      } else if (index === 3) {
