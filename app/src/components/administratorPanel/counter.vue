...
export default {
  ...
  methods: {
    inputChange () {
      if (parseInt(this.$refs.input.value) > parseInt(this.value)) {
    ...
    reduceProductNumber () {
      ...
      this.$refs.input.value = this.counterNumber // inout的value的值方法结束后才给input，会少1，所以先赋值
      this.$emit('input', this.$refs.input.value)
    },
    increaseProductNumber () {
      if (this.counterNumber < this.originalNumber) {
        this.counterNumber++
