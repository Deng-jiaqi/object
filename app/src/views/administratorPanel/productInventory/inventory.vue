      ...
      <div class="inventory" v-if="judgeNum !== 0" v-loading="loading">
        ...
        <div class="header-search">
          <div class="allInventory">
            ...
            <div class="allInventory-choose" v-if="options.length > 0">
              <el-select v-model="value" v-loading="loading" :placeholder="$t('inventory.header.select.placeholder')"
          ...
          <div class="div-search">
            <input class="search" v-model="searchData" @input="search"
                   :placeholder="$t('inventory.header.search')" />
            <span class="iconfont icon-sousuo4"></span>
            <button v-if="getTotal.length>0" @click="editProCP" class="serBtn">
              {{$t('inventory.edit.text')}}({{getTotal.length}})
            </button>
            <el-popover
              ...
              <div class="btn1-bianJi">
                <ul>
                  <li v-for="(item,index) in $t('inventory.edit.text2.list')" :key="index" :data-index="index"
                      @click="clickmsg(item)">{{item}}
                  </li>
                </ul>
              </div>
        ...
        <div class="inventory-table">
          <table v-loading="loading" v-if="trData.length > 0">
            <thead>
            <th>
              <input type="checkBox" class="check_box tui-checkbox" @click="selectProduct(isSelectAll)"
                     v-bind:checked="isSelectAll" />
            </th>
            <th>{{$t('inventory.table.thead.th1')}}</th>
            <th width="50">{{$t('inventory.table.thead.th2')}}</th>
            <th width="50">{{$t('inventory.table.thead.th3')}}</th>
            <th>{{$t('inventory.table.thead.th4')}}</th>
            </thead>
            <tbody style="text-align: center">
            <tr v-for="(item, index) in trData" :key="index">
              <td>
                <input type="checkbox" class="check_box tui-checkbox" v-bind:checked="item.isSelect"
              ...
              <td class="td-imgBox">
                ...
                <div>
                  <p @click="item.attrpid !== 0 ? productDetails(item) : productDes(item)">
              ...
              <span v-if="item.addNum === true">
                    {{ newSetNum = Number(item.num)+ Number(item.storage)}}
              </span>
              ...
              <td>
                <div class="compileNum">
                  <el-button-group style="display: flex">
                    <el-button v-model="item.addNum" @click="addNumFun(item)"
                               :class="[item.addNum === true ? sxColor[0] : sxColor[1]]">
                      {{$t('inventory.table.tbody.btn1')}}
                    </el-button>
                    <el-button v-model="item.setNum" @click="setNumFun(item)"
                               :class="[item.setNum === true ? sxColor[0] : sxColor[1]]">
                      {{$t('inventory.table.tbody.btn2')}}
                    </el-button>
                    <el-input-number v-model="item.num" :min="0" :max="999999" :label="$t('inventory.table.tbody.label1')">
                    </el-input-number>
                  </el-button-group>
                  <el-button class="saveBtn" @click="saveEditNum(item)"> {{$t('inventory.table.tbody.btn3')}}</el-button>
                </div>
              </td>
            </tr>
            </tbody>
        ...
        <el-dialog
          ...
          <div class="updateNum">
            <el-button-group>
              <el-button v-for="(item, index) in this.$t('inventory.btnSet')"
                         :key="index"
                         :class="{ 'compileBtn': active === index }"
                         @click="qieHuanClass(index)">{{item.text}}
              </el-button>
              <el-input-number v-model="setNum" :min="0" :max="999999" :label="$t('inventory.elDialog.label')">
              </el-input-number>
            </el-button-group>
          </div>
          <div class="updateNum-box">
            <p>{{getAttrs.length}}<span>{{$t('inventory.elDialog.text1')}}</span>  </p>
            <ul>
              <li v-for="(item, index) in getAttrs" :key="index">
                <p><span>{{item.goodsname}}</span>-<span>{{item.attr}}</span></p>
                <p>{{item.storage}} <span v-if="setNum !== 0">></span> <span v-if="setNum !== 0" class="spanCss">
        ...
        <el-button type="primary" @click="saveSetNum">{{$t('inventory.elDialog.btn2')}}</el-button>
      </span>
        </el-dialog>
      </div>
      <div v-else>
        ...
        <el-row :gutter="20" class="allProducts-content flexCss">
