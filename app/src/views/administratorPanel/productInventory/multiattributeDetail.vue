  ...
  <div id="MultiattributeDetail" v-loading="loading">
    ...
    <div class="multiple-header">
      <div>
        ...
        <p class="add-black"  @click="backTop"><span class="el-icon-arrow-left"></span>{{$t('mulAttrDetails.header.back')}}</p>
      </div>
              ...
              <el-row>
                <el-col :span="11">
                  <div class="duoShuXing">
                    <div>
                      <ul>
                        <li v-for="(item, index) in multiData"
                            class="qieHuanLi"
                            :key="index"
                ...
                <el-col :span="11">
                  <div class="itemBox1">
                    ...
                    <div>
                      ...
                      <p class="sizeCss"><span>{{multiData.length}}</span>{{$t('mulAttrDetails.content.text2')}} </p>
                    </div>
                  </div>
                </el-col>
              </el-row>
              ...
              <el-row>
                ...
                <el-col :span="8">
                  <div class="changeImg">
                    ...
                    <p>
                      <el-link type="primary" c @click="nowImg">{{$t('mulAttrDetails.content.text5')}}</el-link>
                    </p>
                  </div>
                </el-col>
              </el-row>
          ...
          <div class="multiple-box box4Mul">
            ...
            <div class="genZongNum">
              <el-checkbox v-model="gzCheckbox">
                {{$t('mulAttrDetails.content.siteNum.text4')}}
              </el-checkbox>
              <br>
              <el-checkbox v-if="gzCheckbox === true">
                {{$t('mulAttrDetails.content.siteNum.text5')}}
              </el-checkbox>
            </div>
            <div class="mulBox4">
              ...
              <div>
                ...
                <span class="orangeColor" @click="bjSite">{{$t('mulAttrDetails.content.siteNum.text7')}}</span>
              </div>
            </div>
            <div class="mulBox4">
            ...
            <div class="mulBox4" v-for="(item, index) in warehouseData" :key="index">
              ...
              <div>
                ...
                <el-popover
                    ...
                    trigger="click">
                  <ul class="flexCss">
                    <li v-for="(item,index) in $t('mulAttrDetails.editSetNum')" class="editSetNum" :key="index"
                        :class="{'editSetNumCss':editSetNumActive===index}"
                        @click="editSetStyle(item,index)">{{item.text}}</li>
                  </ul>
                  ...
                  <div class="setCsCss" v-else>
                    <div>
                      ...
                      <div style="width: 130px">
                        ...
                        <el-input v-model="SetnewNum"/>
                      </div>
                    </div>
                  </div>
                  <div class="editeCaoZuo">
                    <el-button @click="item.visibleEditSet = false">{{$t('mulAttrDetails.content.siteNum.btn1')}}</el-button>
                    <el-button @click="saveNewNum(item)" class="blueBtn">{{$t('mulAttrDetails.content.siteNum.btn2')}}</el-button>
                  </div>
                  <span  slot="reference" class="orangeColor">
                        <el-link :underline="false" style="color: #FFA400">{{item.num}}
                        </el-link>
                      </span>
                </el-popover>
              </div>
            ...
            <div class="mulBox4" v-if="warehouseData.length>1">
              ...
              <div>
                <span>0</span>
                <span class="orangeColor" v-if="this.$route.query.attrpid === undefined">{{profit.proNum}}</span>
            ...
            <div v-if="carriageCheck === true">
              <div class="weightBox">
                ...
                <div class="weight-box">
                  <input type="text" class="weight-input" v-model="weightNum">
                  <ul class="unit-ul">
...
<!--              </div>-->
          ...
          <el-form-item>
            <div class="saveMul">
              <el-button @click="saveMul" v-preventReClick="2000">{{$t('mulAttrDetails.content.siteNum.btn2')}}</el-button>
            </div>
          </el-form-item>
        </el-form>
      </div>
      ...
      <el-dialog
        ...
        <span slot="footer" class="dialog-footer">
          <div class="flexBetween">
            <div>
               <el-button type="danger" v-if="goodImg !== null"
                          @click="delImg">{{$t('mulAttrDetails.elDialogImg.text1')}}</el-button>
            </div>
            <div>
          <el-button @click="dialogVisibleImg = false">{{$t('mulAttrDetails.elDialogImg.btn1')}}</el-button>
          <el-button class="editAddImg" @click="uploadHint">
            {{$t('mulAttrDetails.elDialogImg.btn')}}
            <template v-if='imgs.length < maxCount'>
              <input type="file" id='filesEdit' @change='fileChange($event)' multiple="multiple" ref="FileValue">
            </template>
          </el-button>
          <el-button class="blueBtn" style="background-color: #4267B2" @click="saveCompleted">{{$t('mulAttrDetails.elDialogImg.btn2')}}</el-button>
            </div>
          </div>
    </span>
      </el-dialog>
      ...
      <el-dialog
        ...
        <div class="editWarehouse-box2" v-show="judgeSite">
          <p><span class="el-icon-warning"></span>{{$t('mulAttrDetails.elDialogSite.text1')}}</p>
        </div>
        ...
        <ul>
          <li v-for="(item, index) in checkedEdit" :key="index" class="genZongNum">
            ...
            <p v-if="item.checked === false"><span v-if="item.num !==0">{{$t('mulAttrDetails.elDialogSite.text3')}}
              {{item.num}} {{$t('mulAttrDetails.elDialogSite.text4')}}</span></p>
          </li>
        </ul>
        <span slot="footer" class="dialog-footer">
      <el-button @click="dialogVisibleSite = false">{{$t('mulAttrDetails.elDialogSite.btn1')}}</el-button>
      <el-button type="primary" @click="addWarehouse" ref="BtnCss" v-preventReClick="2000">{{$t('mulAttrDetails.elDialogSite.btn2')}}</el-button>
      </span>
      </el-dialog>
    </div>
  </div>
