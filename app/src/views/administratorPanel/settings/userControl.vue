  ...
  <div class="userControl">
    <div class="addSet-header">
      ...
      <p class="add-black" @click="backTop"><span class="el-icon-arrow-left"></span>
        <span>{{$t('userControl.header.back')}}</span></p>
    </div>
    <el-row class="addSet-box">
      <el-col :span="8">
        <h3>{{$t('userControl.content.text1')}}</h3>
        <p>{{$t('userControl.content.text2')}}
        ...
        <div class="set-two-right">
          <div>
            ...
            <div class="userContent">
              ...
              <div>
                <p><el-link type="primary" :underline="false" @click="adminDetail">{{name}}</el-link></p>
                <p>{{$t('userControl.content.text16')}}<span>{{last_login}}</span></p>
              </div>
            </div>
          </div>
         <div></div>
...
<!--        // 员工账户-->
        <div class="set-staff-right">
          <div class="set-staff-box">
            <h3>{{$t('userControl.content.text17')}}<span>{{staffLength}}</span> {{$t('userControl.content.text18')}}</h3>
            <p><el-link type="primary" :underline="false" @click="addUser" v-if="staffLength < 2">{{$t('userControl.content.btn1')}}</el-link></p>
          </div>
          <ul class="setUl">
                  ...
                  <el-avatar :size="50"
                             :src="item.head_img === null
                              ? item.head_img = 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png'
                               : item.head_img" @error="errorHandler">
                    <img src="https://cube.elemecdn.com/e/fd/0fc7d20532fdaf769a25683617711png.png"/>
                  </el-avatar>
                </div>
                <div>
                  <p><el-link type="primary" :underline="false" @click="routerUserDetail(item)">
                    {{item.firstname + item.lastname}}</el-link></p>
                  <p>
                    <span v-if="item.status === -1">{{$t('userControl.content.li1')}}</span>
                    <span v-else-if="item.last_login === null">{{$t('userControl.content.li2')}}</span>
                    <span v-else-if="item.status === 1">{{$t('userControl.content.li3')}}{{item.last_login}}</span>
                  </p>
                </div>
              </div>
              <p>Full access</p>
            ...
            <li class="setUl-li" v-for="(item, index) in staff.wait" :key="'k1' + index">
              <div class="userContent">
                <div class="block">
                  <el-avatar :size="50" :src="item.head_img === null
                   ? item.head_img = 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png'
                    : item.head_img" @error="errorHandler">
                    <img src="https://cube.elemecdn.com/e/fd/0fc7d20532fdaf769a25683617711png.png"/>
                  </el-avatar>
            ...
            <template v-if="unuse.length > 0">
              <li class="setUl-li2" v-if="hideClick">
                ...
                <div class="userContent-li" v-for="(item, index) in unuse" :key="'k2'+ index">
                  <div class="userContent">
                    ...
                    <div>
                      <p><el-link type="primary" :underline="false" @click="routerUserDetail(item)">{{item.firstname + item.lastname}}</el-link></p>
                      <p>
                        <span v-if="item.status === -1">{{$t('userControl.content.li1')}}</span>
                        <span v-else-if="item.last_login === null">{{$t('userControl.content.li2')}}</span>
                        <span v-else-if="item.status === 1">{{$t('userControl.content.li3')}}{{item.last_login}}</span>
              ...
              <el-row>
                ...
                <el-col :span="4">
                  <p>
                    <el-link type="primary" @click="hideClick = !hideClick" :underline="false">
                      {{hideClick === false ? $t('userControl.content.li6') : $t('userControl.content.li7')}}
                    </el-link>
                  </p>
                </el-col>
              </el-row>
