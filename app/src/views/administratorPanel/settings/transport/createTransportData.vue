<template>
  <div class="c-createTransportData">
    ...
    <div class="box-addproduct" v-show='!isdefault'>
      ...
      <div class="c-search">
        <el-input :placeholder="'搜索特定产品'"></el-input>
    ...
    <div class="btn-save-cancel">
      <edg-button :type="'simple'" :text="'取消'" class="btn-cancel" @click.native="cancel" />
      <edg-button :type="'primary'" :text="'确定'" @click.native="save" />
    </div>
    <!-- 特定产品模态窗 -->
    ...
    <div class="modal modal-distribution-area">
      <el-dialog :show-close='false' :title="status==='add'?'创建':'修改'" :visible.sync="distributionAreaVisible" width="36%">
        <div class="c-name">
        ...
        <span slot="footer" class="dialog-footer">
          <edg-button
            class="btn-cancel"
            @click.native="cancelEditDistributableArea"
            :text="'取消'"
          ></edg-button>
          <edg-button
            :type="'primary'"
            @click.native="CreateFixDeliverableAreas"
            :text="'确定'"
          ></edg-button>
        </span>
      </el-dialog>
  ...
  methods: {
    async save () {
      ...
      let res = await this.ajax.post(
        'http://192.168.5.108/tp5.0/public/index.php/basic/Transport/add',
        sendData
      )
      messageAlert(res)
    ...
    CreateFixDeliverableAreas () { // 增加或修改可配送区域
      ...
      switch (this.status) {
        ...
        case 'fix':
          ...
          this.selectedDistributionArea[this.indexofinEditSelectedDistributionArea] = {
            name: this.templateName,
            zhCountryName: zhCountryName,
            enCountryName: enCountryName,
            country: SelectedCountryArr.toString(),
      ...
      this.IdSelectedCountryArr = this.IdSelectedCountryArr.concat(SelectedCountryArr)
      this.AllIdSelectedProviceArr = this.AllIdSelectedProviceArr.concat(SelectedProviceArr).concat(underCountryProviceArr)
    ...
    openEditDistributableArea (item, index) {
      ...
      this.distributionArea.forEach(item => {
        if (this.itemCountryArr.indexOf(item.id) !== -1) { // 国家被选中了
          item.isSelect = true
          item.disable = false
          if (item.child) {
            item.child.forEach(itemTwo => {
        ...
        if (this.itemCountryArr.indexOf(item.id) === -1 && item.child) { // 国家没被选中，省呗选中了
          let disableLenth = 0
          item.child.forEach(itemThree => {
    ...
    addDisable () { // 在其它资料中选中项加上disable属性
      ...
      this.distributionArea.forEach(item => {
        ...
        if (item.child) {
          ...
          if (lenth === disableNum && this.IdSelectedCountryArr.indexOf(item.id) === -1) {
